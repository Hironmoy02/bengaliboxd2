...
export default function LoginPage() {
  ...
  const handleSandboxLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (googleLoginUser.rejected.match(result)) {
        setError((result.payload as string) || 'Sandbox login failed');
      } else {
        router.push('/');
      }
    } catch {
      setError('Sandbox login failed');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh', px: 2 }}>
        <Paper sx={{ width: '100%', maxWidth: 440, p: 4, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            ...
            <Typography color="text.secondary">Use Google Authentication to sign in or register instantly</Typography>
          </Box>
            ...
            <Stack spacing={3}>
              <Box sx={{ p: 2, background: 'rgba(0,102,204,0.06)', borderRadius: 2, border: '1px solid rgba(0,102,204,0.15)' }}>
                ...
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', lineHeight: 1.4 }}>
                  To enable real Google login, configure <code>NEXT_PUBLIC_GOOGLE_CLIENT_ID</code> in your <code>.env.local</code> file.
                </Typography>
              </Box>
              ...
              <form onSubmit={handleSandboxLogin}>
                <Stack spacing={2}>
                  ...
                  >
                    {isSubmitting ? 'Authenticating...' : 'Sign in with Google (Sandbox)'}
                  </Button>
                </Stack>
              </form>
            </Stack>
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', textAlign: 'center', lineHeight: 1.4 }}>
            Bengaliboxd uses Google SSO to verify user identities. By signing in, your account is automatically created if it doesn't already exist.
          </Typography>
        </Paper>
      </Box>
    </>
